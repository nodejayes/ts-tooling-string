...
describe('String Extensions Tests', () => {
    ...
    describe('[Method]: RegExpEscape', () => {
        it('escape RegExp', () => {
            assert.equal('[google](https://google.com/)'.RegExpEscape!(), '\\[google\\]\\(https://google\\.com/\\)');
        });
    });
    ...
    describe('[Method]: Words', () => {
        ...
        it('with unicode character', () => {
            assert.deepEqual('alpha is the \u03A9 or so'.Words!(), [
                'alpha', 'is', 'the', '\u03A9', 'or', 'so'
            ]);
        });
        it('use a custom pattern', () => {
            assert.equal('abcd'.Words!(undefined, 'a').length, 1);
            assert.equal('abcd'.Words!(undefined, 'a')[0], 'a');
        });
        it('add a word filter', () => {
            assert.equal('hello is a word of a sentence'
                .Words!(e => [
                    'hello', 'word', 'sentence'].indexOf(e) > -1
                ).length, 3);
            assert.equal('hello is a word of a sentence'
                .Words!(e => [
                    'hello', 'word', 'sentence'].indexOf(e) > -1
    ...
    describe('[Method]: Pad', () => {
        it('add custom letter on both sides', () => {
            ...
            assert.equal('abc'.Pad!(10, '_-'), '-_-abc_-_-');
        });
    ...
    describe('[Method]: Split', () => {
        ...
        it('nothing is empty array', () => {
            assert.deepEqual(''.Split!('z'), []);
        });
